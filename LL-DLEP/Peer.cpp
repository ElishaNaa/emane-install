...
Peer::handle_peer_initialization(ProtocolMessage & pm)
{
    ...
    if (status_message != "")
    {
        ...
        // the receiver of incosistent information MUST issue a Session
        // Termination Message containing a Status Data Item with status set
        // to 'Invalid Data' and transition to the Session Termination state.
        terminate(ProtocolStrings::Invalid_Data, status_message);
        return;
    }
...
Peer::handle_destination_up(ProtocolMessage & pm)
{
    ...
    if (! already_have_this_dest)
    {
        ...
        {
            // following the RFC 8175 protocol (13.8.1),
            // MAY issue the appropriate response message containing Ststus Data Item.
            ostringstream msg;
            ...
            // the receiver of inconsistent message MUST continue with session processing,
...
Peer::handle_destination_announce(ProtocolMessage & pm)
{
    ...
    if (! already_have_this_dest)
    {
        ...
        msg << " geting destination data";
        LOG(DLEP_LOG_INFO, msg);

        if (dlep->local_pdp->getDestinationData(destination_mac, &ddp))
        {
            msg << " geting destination data YES";
            LOG(DLEP_LOG_INFO, msg);
    ...
    {
        msg << " What's the right status for a redundant Destination Announce?";
...
Peer::handle_destination_update(ProtocolMessage & pm)
{
    ...
    if (good_destination)
    {
        ...
        std::string status_message =
            validate_ip_data_items(update_data_items, dlep->local_pdp->get_ip_data_items());
        if (status_message != "")
        {
            // following the RFC 8175 protocol (13.8.1),
            // MAY issue the appropriate response message containing Ststus Data Item.
            ...
            // following the RFC 8175 protocol,
            // the receiver of inconsistent message MUST continue with session processing,
            // therefor, therefore there is no place for 'return' 
        }
