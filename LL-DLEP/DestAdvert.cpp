    ...
    else
    {
        ...
        for (auto dataItems : ipParameters)
        {
    		...
    		for (auto ipdi : dataItems)
    		{
    	        ...
    	        //if ip add then we need to add all new and unique ip data items
...
void DestAdvert::add_dataitem(const LLDLEP::DataItem & di)
{
    ...
    DataItems* selfDataItemes(nullptr);
    if (di.name() == ProtocolStrings::IPv4_Address)
    {
    	selfDataItemes = &localPeerIpv4DataItems;
    }else if (di.name() == ProtocolStrings::IPv4_Attached_Subnet)
    {
    	selfDataItemes = &localPeerIpv4SnDataItems;
    }else if (di.name() == ProtocolStrings::IPv6_Address)
    {
    	selfDataItemes = &localPeerIpv6DataItems;
    }else if (di.name() == ProtocolStrings::IPv6_Attached_Subnet)
    {
    	selfDataItemes = &localPeerIpv6SnDataItems;
    }

    DataItems ipdisRemove;
	for (auto diRemove : *selfDataItemes)
	{
	...
	{
		selfDataItemes->erase( std::remove( selfDataItemes->begin(), selfDataItemes->end(), diRemove ), selfDataItemes->end() );
	}
	selfDataItemes->push_back(di);
}
                                           ...
                                           const DataItems & data_items)
{
    ...
    if (iter != dest_advert_db.end())
    {
        ...
        //add all new data items which are not ip
        ...
        //add ip data itemes that we save aside
